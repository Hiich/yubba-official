...
function RowTokenInfo({
  ...
  return (
    <div
      className={clsx(
        'flex flex-row min-h-[14px]',
        border && 'border-b border-white border-dotted',
      )}
    >
      <div className="flex justify-start w-6 pt-6">
        <div className="h-2 w-2 rounded-full bg-[#e74094]"></div>
      </div>
      <div className="flex flex-1 py-4 text-white">{info}</div>
    </div>
  )
}
...
function Trait({
  ...
  return (
    <div
      className={clsx(
        'flex justify-center items-center h-9 rounded font-medium text-secondary px-6 bg-[#fbfafa] border border-[#979797]',
        className,
      )}
      style={style}
    >
  ...
  return (
    <>
      <div className="flex items-center h-[60px]">
        ...
        <div className="flex flex-row justify-between flex-1">{children}</div>
      </div>
      {underPlus && (
        <div className="flex items-center h-2">
          <div className="flex w-20"></div>
          <div className="flex-grow ml-8 font-extrabold font-title">+</div>
...
function RowRarity({ category, number }: { category: string; number: string }) {
  return (
    <div className="flex items-center h-12 border-b-[1px] border-dotted border-[#342c30]">
      <div className="flex flex-1 w-full font-bold">{category}</div>
  ...
  return (
    <>
      ...
      <section className="bg-[#b2c2d3]">
        <Container>
          <div
            className="rounded pt-10 pb-10  px-16 w-full mb-28 md:w-[450px]"
            style={{
              backgroundImage:
                'linear-gradient(152deg, #473c41 10%, #342c30 79%)',
            }}
          >
            ...
            <RowTokenInfo
              info="8% Yubba tax on secondary market sales used for Yubba World housekeeping"
              border={false}
            />
          </div>
          ...
          <div className="mt-8 mb-10 text-secondary">
            Yubba characters have been designed with hundreds of different
            traits and attributes and generated from 5-6 composition layers.
          </div>
          ...
          <div className="flex flex-wrap w-full space-x-0 space-y-20 sm:pt-10 sm:space-y-0 sm:flex-nowrap sm:space-x-20 text-secondary">
            ...
            <div className="w-full sm:w-1/2">
              <div className="mb-4 text-xl font-bold">Traits composition</div>

              <div className="flex items-center h-10 border-dotted border-b-[1px] border-[#342c30] mb-2">
                <div className="flex w-20">Layers</div>
                <div className="flex flex-1">Traits</div>
              </div>

              <RowComposition layer={6} underPlus={2}>
                <Trait>Headdress</Trait>
                <Trait
                  className="w-[103px]"
